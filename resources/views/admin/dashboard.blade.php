...
<body>
    <div class="container">
        ...
        <section class="filter-container">
            ...
            <form action="{{ route('FiltrarAdminDashboard') }}" method="get" class="filter-form">
            ...
            @else
                @foreach($adminproperties as $adminproperty)
                    <div class="property-item">
                        @foreach ($adminproperty->images as $image)
                            <img src="{{ asset($image->image_path) }}" alt="Imagen de la propiedad">
                        @endforeach
                        <h3>{{ $adminproperty->titulo }}</h3>
                        <p><strong>Dirección:</strong> {{ $adminproperty->direccion }}</p>
                        <p><strong>Precio:</strong> ${{ number_format($adminproperty->precio, 2) }}</p><br>
                        <b><center><a href="{{ route('property.show', $adminproperty->id) }}" class="btn btn-info">Ver más...</a></center></b>
                    </div>
                @endforeach
            @endif
        </section>
